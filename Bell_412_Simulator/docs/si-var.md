You said
Rewrite complete adi.lua (reciever) dont miss anything. reciver code should recieve encoder values and move a adi accordingly

At reciever end adi.lua is running.

Create new lua code at sender it should send the ecoders values atta5tched to aurdinos at pins at channel g:

ADI Left: Right encoder (A6 and A2) and left Encoder (D12  and unknown)
...
Gemini said
Here is the complete solution. I have split your monolithic code into a Sender Gauge (handling the Arduino hardware inputs and broadcasting the values) and a Receiver Gauge (handling the UI images and listening to both the Simulator and the Sender).

We will use si_variable_create on the Sender to broadcast the gvar1 (cage) and gvar2 (heading) values, and si_variable_subscribe on the Receiver to catch them and move the UI elements exactly as your original code did.

1. The Sender Code (Hardware Inputs)
Create a new blank instrument for your hardware, name it something like "ADI Hardware Sender", and paste this code into its logic.lua.

Lua
-- =============================================================================
-- SENDER GAUGE: BELL 412 ADI HARDWARE
-- Encoders (Channel G)
-- =============================================================================
...
-- 1. Create SI variables to broadcast encoder states to the Receiver gauge
local si_cage_id    = si_variable_create("si_adi_cage", "INT", gvar1)
local si_heading_id = si_variable_create("si_adi_heading", "INT", gvar2)

-- 2. Shared callback: CAGE KNOB
local function cage_knob_callback(direction)
    gvar1 = gvar1 + direction
    gvar1 = var_cap(gvar1, -12, 12)
    ...
    -- Broadcast to Receiver gauge
    si_variable_write(si_cage_id, gvar1)
    print("SENDER: Cage Knob -> " .. gvar1)
end

-- 3. Shared callback: HEADING KNOB
local function heading_knob_callback(direction)
    gvar2 = gvar2 + direction
    gvar2 = var_cap(gvar2, -30, 30)
    ...
    -- Broadcast to Receiver gauge
    si_variable_write(si_heading_id, gvar2)
    print("SENDER: Heading Knob -> " .. gvar2)
end
...
-- ADI Left - Left Encoder (Heading Knob): D12 & D13 (DUMMY D13, replace when known)
hw_dial_add("ARDUINO_MEGA2560_G_D12", "ARDUINO_MEGA2560_G_D13", "TYPE_1_DETENT_PER_PULSE", heading_knob_callback)
2. The Receiver Code (adi.lua)
Replace the contents of your existing adi.lua with the following code. It retains all your image logic and simulator subscriptions, but replaces the hardware dials with Air Manager Inter-Instrument Subscriptions.

Lua
-- =============================================================================
-- RECEIVER GAUGE: BELL 412 - ADI (Attitude Direction Indicator)
-- Listens to Simulator Data & Sender Gauge (Encoders)
-- =============================================================================
...
-- =============================================================================
-- 3. INTER-INSTRUMENT COMMUNICATION (Receiving from Sender Gauge)
-- =============================================================================
...
function on_heading_received(gvar2_val)
    -- Visually rotate the bank pointer by the offset 
    rotate(img_ring, gvar2_val)
end

-- Subscribe to the variables created by the Sender gauge
si_variable_subscribe("si_adi_cage", "INT", on_cage_received)
si_variable_subscribe("si_adi_heading", "INT", on_heading_received)

-- =============================================================================
-- 4. ATTITUDE DISPLAY LOGIC (From Simulator)
-- =============================================================================
function PT_atitude_xpl(roll, pitch, slip, APmode, GS_1, GS_2, GS_hsi, LC_1, LC_2, LC_hsi, glide_1, glide_2, glide_hsi, loc_1, loc_2, loc_hsi, ADIfail, FDpitch, FDroll)
...
Testing the setup: Load both instruments up, connect the Air Manager plugin to your simulator, and spin the Arduino dials. The values will route directly into the simulator variables, while simultaneously triggering the img_fd_horizontal and img_ring visual movements on the Receiver gauge display!